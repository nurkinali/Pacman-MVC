 ...
 * <h1>Ghost</h1>
 *
 * <p>A {@link Ghost} is a {@link MovableGrid} in a {@link GameMap}. It can move randomly. When it
...
public class Ghost extends MovableGrid implements Runnable {
   ...
   *
   * <p>This constructor sets the {@link Ghost} in the given position of the given {@link GameMap}, sets
   * a random image of {@link Ghost}, and gives a initial {@link Direction#UP} to move to.
   *
   * @param map the {@link GameMap} where this {@link Ghost} stays
   * @param row the row index in the {@link GameMap} where this {@link Ghost} stays, starting from 0
   * @param column the column index in the {@link GameMap} where this {@link Ghost} stays, starting from
   *     0
   */
  public Ghost(GameMap map, double row, double column) {
    super(map, row, column, MovableGridType.GHOST);

    // set a random ghost image
    int randomIndex = new Random().nextInt(ResourcesFileName.IMAGE_GHOSTS.size());
    int index = 0;
    for (String image : ResourcesFileName.IMAGE_GHOSTS) {
  ...
  /**
   * Returns the value of {@link PlayerScore} of this {@link Ghost}.
   *
   * <p><b>Note:</b> currently the value is constant as {@code 5}.
   *
   * @return the value of {@link PlayerScore} of this {@link Ghost}
   */
  public int getValue() {
    // TODO: 2018-12-12
